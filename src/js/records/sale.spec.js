...
describe('record.sale', () => {
  ...
  it('should properly parse defaultQuoteAsset field', () => {
    expect(saleRecord.defaultQuoteAsset)
      .to.equal(recordResponses.sale.default_quote_asset)
  })
  it('should properly parse quoteAssets field', () => {
    expect(saleRecord.quoteAssets)
      .to.deep.equal(recordResponses.sale.quote_assets.quote_assets)
  })
  ...
  it('should properly parse descriptionID field', () => {
    expect(saleRecord.descriptionID)
      .to.equal(recordResponses.sale.details.description)
  })
  it('should properly parse shortDescription field', () => {
    expect(saleRecord.shortDescription)
      .to.equal(recordResponses.sale.details.short_description)
  })
  it('should properly parse youTubeVideoId field', () => {
    expect(saleRecord.youTubeVideoId)
      .to.equal(recordResponses.sale.details.youtube_video_id)
  })
  ...
  it('should properly parse investors field', () => {
    expect(saleRecord.investors)
      .to.equal(recordResponses.sale.statistics.investors)
  })
  it('should properly parse averageInvestment field', () => {
    expect(saleRecord.averageInvestment)
      .to.equal(recordResponses.sale.statistics.average_amount)
  })
  ...
  it('getter quoteAssetCodes should properly derive data', () => {
    expect(saleRecord.quoteAssetCodes).to.deep.equal(['BTC', 'ETH'])
  })
  it('getter quoteAssetPrices should properly derive data', () => {
    expect(saleRecord.quoteAssetPrices).to.deep.equal({
      'BTC': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'BTC').price,
      'ETH': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'ETH').price
    })
  })
  it('getter currentCaps should properly derive data', () => {
    expect(saleRecord.currentCaps).to.deep.equal({
      'BTC': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'BTC').current_cap,
      'ETH': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'ETH').current_cap
    })
  })
  it('getter totalCurrentCaps should properly derive data', () => {
    expect(saleRecord.totalCurrentCaps).to.deep.equal({
      'BTC': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'BTC').total_current_cap,
      'ETH': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'ETH').total_current_cap
    })
  })
  it('getter hardCaps should properly derive data', () => {
    expect(saleRecord.hardCaps).to.deep.equal({
      'BTC': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'BTC').hard_cap,
      'ETH': recordResponses.sale.quote_assets.quote_assets
        .find(asset => asset.asset === 'ETH').hard_cap
    })
  })
  // eslint-disable-next-line
