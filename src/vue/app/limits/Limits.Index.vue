<template>
  <div class="limits">
    <template v-for="(asset, i) in Object.keys(accountBalances)">
      <md-card class="limits__content" :key="i">
        ...
        <md-table class="limits__table">
          <md-table-row class="limits__row">
            <md-table-head>{{ i18n.lim_transaction_type_title() }}</md-table-head>
            <md-table-head>{{ i18n.lim_daily() }} {{ i18n.lim_left_total() }}</md-table-head>
            <md-table-head>{{ i18n.lim_weekly() }} {{ i18n.lim_left_total() }}</md-table-head>
            <md-table-head>{{ i18n.lim_monthly() }} {{ i18n.lim_left_total() }}</md-table-head>
            <md-table-head>{{ i18n.lim_annual() }} {{ i18n.lim_left_total() }}</md-table-head>
          </md-table-row>
          <template v-for="([operation, name],i) in Object.entries(limitOperations)">
            <md-table-row class="limits__table-row" :key="i">
              <md-table-cell class="limits__table-cell limits__operation-type">{{ operation }}</md-table-cell>
        ...
        <div class="limits__table-action-btn-outer">
          <md-button class="limits__table-action-btn md-primary" @click="showForm(asset)">{{i18n.lim_change_limits()}}</md-button>
        </div>
      </md-card>
    </template>
    <div class="limits__requests-list"
    ...
    <modal v-if="showDialog"
          ...
          maxWidth="30rem">
      <limits-manager :limits="selectedLimits"
                      :asset="asset"
                      @close-request="showDialog = false"/>
    </modal>
  </div>
