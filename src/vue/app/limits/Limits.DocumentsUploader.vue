<template>
  <div class="limits-manager">
    <h2 class="limits-manager__title">{{ i18n.lim_change_limits_title({assetName: assetName, asset: request.tokenCode}) }}</h2>
    <template v-if="view.mode === VIEW_MODES.upload">
      <detail :prop="i18n.lbl_request_creation_date()"
      ...
      <div class="limits-manager__document-wrapper"
          v-for="(item, i) in documents" :key="i">
        <detail :prop="i18n.lbl_document_type()"
                :value="item.label"
                class="limits-manager__document-type limits-manager__detail-row"/>
        <detail :prop="i18n.lbl_note()"
                :value="item.description"
                class="limits-manager__document-description limits-manager__detail-row"/>
        <div class="limits-manager__file-field-wrapper">
          <file-field class="limits-manager__file-field"
    ...
    <template v-if="view.mode === VIEW_MODES.pending">
      ...
      <div class="limits-manager__action-btn-outer">
          <md-button class="limits-manager__action-btn md-primary"
                      ...
                      :disabled="isPending">{{i18n.lbl_ok()}}</md-button>
      </div>
    </template>
  </div>
</template>
...
<script>
  ...
  import { documentTypes,
    LIMITS_REQUEST_TYPE,
    LIMIT_OPS,
    LIMIT_OPS_STR } from '../../../js/const/const'
  ...
  export default {
    ...
    computed: {
      ...
      assetName () {
        return get(this.accountBalances[this.request.tokenCode], 'asset_details.details.name')
      },
      operationType () {
        const limitOp = Object.keys(LIMIT_OPS).find(key => LIMIT_OPS[key] === this.request.details.operationType)
    ...
    methods: {
      ...
      async uploadDocuments () {
        await Promise.all(this.documents.map(doc => this.uploadSingleDocument(doc.file)))
      },
      async uploadSingleDocument (document) {
        if (document.isUploaded) return
        const documentKey = await fileService.uploadFile(document.getDetailsForUpload())
...
<style lang="scss" scoped>
  .limits-manager__document-wrapper,
