<template>
  <div class="limits-manager">
    ...
    <div class="limits-manager__select-wrp">
      <select-field v-model="form.operationType"
    ...
    <md-table class="limits-manager__table">
      ...
      <md-table-row class="limits-manager__table-row">
        ...
        <md-table-cell class="limits-manager__table-cell">
          <editable-input-field :value="form.limits.dailyOut"
                                ...
                                align="right"
                                min="0"
                                :errorMessage="errorMessage('daily limit amount')"
      ...
      <md-table-row class="limits-manager__table-row">
        ...
        <md-table-cell class="limits-manager__table-cell">
          <editable-input-field :value="form.limits.weeklyOut"
                                :step="DEFAULT_INPUT_STEP"
                                :fieldId="`2`"
                                type="number"
                                ...
                                iconSize="18"
                                align="right"
                                :errorMessage="errorMessage('weekly limit amount')"
      ...
      <md-table-row class="limits-manager__table-row">
        ...
        <md-table-cell class="limits-manager__table-cell">
          <editable-input-field :value="form.limits.monthlyOut"
                                :step="DEFAULT_INPUT_STEP"
                                :fieldId="`3`"
                                type="number"
                                ...
                                iconSize="18"
                                align="right"
                                :errorMessage="errorMessage('monthly limit amount')"
      ...
      <md-table-row class="limits-manager__table-row">
        ...
        <md-table-cell class="limits-manager__table-cell">
          <editable-input-field :value="form.limits.annualOut"
                                :step="DEFAULT_INPUT_STEP"
                                :fieldId="`4`"
                                type="number"
                                ...
                                iconSize="18"
                                align="right"
                                :errorMessage="errorMessage('annual limit amount')"
    ...
    methods: {
      ...
      async submit () {
        ...
        try {
          ...
          const limits = Object.keys(this.form.limits).filter(item => item).reduce((newObj, item) => {
            return {
              ...newObj,
              [item]: this.form.limits[item] ? this.form.limits[item] : DEFAULT_MAX_AMOUNT
            }
          }, {})
          const note = this.form.note
          const requestType = LIMITS_REQUEST_TYPE.initial
          const operationTypeStr = Object.keys(LIMIT_OPS_STR).find(key => LIMIT_OPS_STR[key] === this.form.operationType)
      ...
      validateForm () {
        ...
        if (form.annualOut) {
          if (+form.annualOut < 0) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_annual_fall_behind_msg())
            return false
          } else if (!form.dailyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_daily_validation_msg())
            return false
          } else if (!form.weeklyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_weekly_validation_msg())
            return false
          } else if (!form.monthlyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_monthly_validation_msg())
            return false
          } else if (+form.annualOut < +form.monthlyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_monthly_exceed_msg())
            return false
          }
        }
        if (form.monthlyOut) {
          if (+form.monthlyOut < 0) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_monthly_fall_behind_msg())
            return false
          } else if (!form.dailyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_daily_validation_msg())
            return false
            ...
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_weekly_exceed_msg())
            return false
          }
        }
        if (form.weeklyOut) {
          if (+form.weeklyOut < 0) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_weekly_fall_behind_msg())
            return false
          } else if (!form.dailyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_daily_validation_msg())
            return false
          } else if (+form.weeklyOut < +form.dailyOut) {
            EventDispatcher.dispatchShowErrorEvent(i18n.limit_daily_exceed_msg())
            return false
          }
        }
        if (form.dailyOut && +form.dailyOut < 0) {
          EventDispatcher.dispatchShowErrorEvent(i18n.limit_daily_fall_behind_msg())
          return false
        }
        if (!form.dailyOut && !form.weeklyOut && !form.monthlyOut && !form.annualOut) {
          EventDispatcher.dispatchShowErrorEvent(i18n.limit_validation_msg())
          return false
        }
        return true
      },
